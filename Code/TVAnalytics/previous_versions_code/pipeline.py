...
def pipeline_tunning_param(n_week_train=15,start_year=2015,start_month=1):
    """
    ...
    inicio=timer()
    train_model(df_X,df_Y,num_model,permutations_dicts,list_tuple_week,index_year_week_train,list_index,list_programa,preprocessing=0)
    fin= (timer()-inicio)/60
...
def train_model(df_X,df_Y,num_model,permutations_dicts,list_tuple_week,index_year_week_train,list_index,list_programa,preprocessing=0):
    ...
    #for t in range(1, 2):
        ...
        #for target in range(1):
            ...
            Y_test.to_csv("output\week_"+str(t)+"\model_predict_xgb_tunning_target_"+str(target)+"_pre_"+preprocessing+".csv")
            error_xgb_tunning = pd.DataFrame.from_records(list_errores).apply(
                    lambda x: x.explode() if x.name in list(range(num_model*2)) else x)
            error_xgb_tunning["week"] =list_tuple_week[list_index.index(index_year_week_train)+1:]
            error_xgb_tunning.to_csv("output\week_"+str(t)+"\error_predict_xgb_tunning_target_"+str(target)+"_pre_"+preprocessing+".csv")
